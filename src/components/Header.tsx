...
export default function Header() {
  ...
  return (
    <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 bg-[#469695] text-white px-4 py-2 rounded">
        Skip to main content
      </a>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="font-bold text-[#0A3660] text-xl">
            The Postpartum OT
          </Link>

          {/* Desktop nav */}
          <nav className="hidden md:flex items-center gap-6 text-sm font-semibold text-[#0A3660]">
            ...
            <div className="relative group">
              <button
                ...
                onClick={() => setClinicianOpen(!clinicianOpen)}
                aria-expanded={clinicianOpen}
              >
                For Clinicians <ChevronDown size={14} />
              </button>
              <div className="absolute top-full left-0 mt-2 w-52 bg-white shadow-lg rounded-lg py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
            ...
            <Link href="/contact" className="bg-[#469695] text-white px-4 py-2 rounded-lg hover:bg-[#3a7e7d] transition-colors">
              Contact
            </Link>
          </nav>
          ...
          {/* Mobile toggle */}
          <button
            className="md:hidden text-[#0A3660]"
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 pb-4">
          <nav className="flex flex-col gap-3 pt-4 text-sm font-semibold text-[#0A3660]">
            ...
            <Link href="/contact" onClick={() => setOpen(false)} className="bg-[#469695] text-white px-4 py-2 rounded-lg text-center">Contact</Link>
          </nav>
        </div>
      )}
    </header>
